...
{
    public class StockMovementRepository : EntityRepositoryBase<StockMovement, ApplicationDbContext>, IStockMovementRepository
    {
        ...
        public async Task<bool> SetActiveAsync(int id)
        {
            try
            {
                var active = await _context.Set<StockMovement>().Where(i => i.Id == id).FirstOrDefaultAsync();
                if (active != null)
                {
        ...
        public async Task<bool> SetDeletedAsync(int id)
        {
            try
            {
                var deleted = await _context.Set<StockMovement>().Where(i => i.Id == id).FirstOrDefaultAsync();
                if (deleted != null)
                {
